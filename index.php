...
<form method="GET">
	<select multiple name="route_id[]" style="height:90; width:600;">
		...
		WHERE ' . $select . ' AND r.route_id = t.route_id AND t.trip_id = st.trip_id AND s.stop_id = st.stop_id
		ORDER BY r.route_long_name , r.route_short_name'; ?>
		<?php $sql = $db->query($sql0); ?>
		<?php foreach ($sql as $row) : ?>
			<option value="<?php echo ($row["route_id"]) . '" '; ?>><?php echo $row["route_long_name"] . " >>> " . $row["route_short_name"]; ?></option>
		<?php endforeach; ?>
	</select>
...
<form method="GET">
	<select multiple name="stop_id[]" style="height:190; width:600;">
		...
		<?php $sql = $db->query($sql0); ?>
		<?php foreach ($sql as $row) : ?>
			<option value="<?php echo ($row["stop_id"]) . '" '; ?>><?php echo $row["stop_name"] . '発 - 行先' . $row["stop_headsign"]; ?></option>
		<?php endforeach; ?>
	</select>
...
<form method="GET">
	<select multiple name="service_id[]" style="height:90; width:600;">
		...
		<?php foreach ($sql as $row) : ?>
			...
			<option value="<?php echo $row['service_id'] . '" '; ?>><?php echo $row['service_id'] ; ?></option>
		<?php endforeach; ?>
	</select>
...
<form method="GET">
	<select multiple name="trip_id[]" style="height:50; width:600;">
		...
		<?php $sql = $db->query($sql0); ?>
		<?php foreach ($sql as $row) : ?>
			<option value="<?php echo $row['trip_id'] . '" '; ?>><?php echo $row['trip_id']; ?></option>
		<?php endforeach; ?>
	</select>
