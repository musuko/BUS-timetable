...
$array = glob("../mini/*.txt");  //フォルダー内のファイル名を取得し配列に入れる ($array[0]="agency_jp.txt)
$data = file("../key/primary.txt", FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES); // $data: テーブル名と主キーカラムの配列。
$cntmax = 80000;    //読み込み上限行数

$tablename = "";
foreach ($array as $filename) {             //ファイル名の配列を順番にテキストで読み込む
    ...
    // $tablename = substr($filename, 8, $len - 4 - 8); // -4は .txtの分。-5は、0から数えて5文字目から表示の分。mini/ を取り除く。
