    ...
    foreach ($info_array as $value) {       // データファイル(.txt)の行ごとの配列を、行ごとに文字列で読み込む
        ...
        foreach ($info_array0 as $value0) {     //　行方向の配列を、一つずつ読み込む
            if ($j === 0) {                     //先頭行の場合、(すなわちカラム行の場合)
                if ($value0 === "stop_sequence") {
                    $txt .= trim($value0) . ' int,';        //数字stop_seuenceにはintを付ける
                } else {
                    $txt .= trim($value0) . ' varchar(100),';   //残りは文字
                }
            } else {
                if ($k !== 4) {                         //load_stop_times.php専用
                    $txt .= "'" . $value0 . "',";      //load_stop_times.php専用
                } else {                               //load_stop_times.php専用
                    $txt .= $value0 . ",";
                }
            }
